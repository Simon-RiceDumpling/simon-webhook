...
public class HttpclientExecutor {
    ...
    private static <T> ExecuteResult<T> execute(HttpMethodBase httpMethod, String url, Class<T> tClass) {
        try {
            ...
            log.debug("url->:【{}】响应结果:【{}】 ", url, jsonObject.toString().length() > 1000 ? httpCode : jsonObject);
            // 转换为目标类型
            T data = JSONObject.parseObject(rawResponse, tClass);
            JSONObject headers = headersToJson(httpMethod.getResponseHeaders());
            return ExecuteResult.success(httpCode, data, rawResponse,headers.toJSONString());
        } catch (Exception e) {
            e.printStackTrace();
