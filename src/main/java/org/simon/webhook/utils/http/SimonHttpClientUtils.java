...
import org.simon.webhook.utils.http.executor.HttpclientExecutor;
...
@Component
public class SimonHttpClientUtils {
    ...
    public static class HttpCall<T> {
        ...
        public ExecuteResult<T> execute() {
            if (body != null && !body.isEmpty()) {
                ...
                return HttpclientExecutor.doExecute(method, url, headers, bodyParams, responseClass);
            } else {
                return HttpclientExecutor.doExecute(method, url, headers, params, responseClass);
            }
        }
    }
    ...
    public void example() {
        // 超简洁的调用方式
        ExecuteResult<JSONObject> result = SimonHttpClientUtils.call(JSONObject.class)
                .get("https://api.example.com/data")
                .header("Authorization", "Bearer token")
                .param("id", "123")
                .execute();

        ExecuteResult<JSONObject> postResult = SimonHttpClientUtils.call(JSONObject.class)
